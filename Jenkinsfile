...
pipeline {
    ...
    environment {
        ...
        GITHUB_REPO_NAME = 'infra-jenkins'
        ...
        stage('Terraform Validate') {
            steps {
                script {
                    ...
                    try {
                        def result = sh(script: 'terraform init && terraform validate', returnStatus: true)
                        if (result != 0) {
                            updateGitHubStatus('terraform-validate', 'failure', 'Terraform Validate check failed', env.PR_COMMIT_SHA)
                            error('Terraform validate check failed!')
                        }
                        echo "Terraform validate succeeded. Updating GitHub status to success."
                        updateGitHubStatus('terraform-validate', 'success', 'Terraform Validate check passed', env.PR_COMMIT_SHA)
                    } catch (Exception e) {
                        echo "Terraform validate failed: ${e.message}"
